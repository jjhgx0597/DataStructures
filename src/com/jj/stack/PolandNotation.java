...
public class PolandNotation {
    public static void main(String[] args) {
        ...
        System.out.printf("%s=%d",expression,calculate(parseSuffixExpression));
/*
        //先定义一个逆波兰表达式
        //(3+4)×5-6 =>3 4 + 5 × 6 - =>29
...
*/
    ...
    public static List<String> parseSuffixExpression(List<String> list){
        ...
        //因此比较麻烦，这里我们就把不用Stack<String>，而是直接用List<String>
//        Stack<String> s2 = new Stack<>();
        ...
        for (String item : list) {
            ...
            if (item.matches("\\d")){
                ...
                //当item的优先级小于等于s1栈顶的运算符，将s1栈顶的运算符弹出并加入到s2中，再次转到（4.1）
                //与s1中新的栈顶运算符相比较
                //问题：缺少一个比较优先级高低的方法
    ...
    public static List<String> toInfixExpressionList(String s){
        ...
        String str;//对多位数的拼接
        char ch;//每遍历一个字符，就放入到ch
        do {
            //如果ch是一个非数字，就需要加入到list里
    ...
    public static int calculate(List<String> list){
        ...
        for (String item : list) {
            if (item.matches("\\d+")) {//数字，直接入栈
                stack.push(item);
            }else {
                //如果是符号，出栈2个数字，运算，将结果入栈
